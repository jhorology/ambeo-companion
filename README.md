...
### 1. 入力監視の許可（メディアキー連動に必須）
Magic Keyboard や Mac の音量キー（メディアキー）を `CGEventTap` によりインターセプトして AMBEO サウンドバーへ転送するため、macOS の **「入力監視」（Input Monitoring）** の許可が必要です。

1. **初回起動時**:
   - macOS の **「システム設定 > プライバシーとセキュリティ > 入力監視」** を開きます。
   ...
   *(※ お使いの環境や macOS のバージョンによっては「アクセシビリティ」側にも許可が必要な場合があります)*
...
2. **アプリ更新時・再ビルド時の注意（音が動かない場合の対処法）**:
   ...
   - **メディアキーのフックが効かない場合**:
     - 「システム設定 > プライバシーとセキュリティ > 入力監視」で一度 **OFF にしてから再度 ON** に切り替えてください（必要に応じてアプリを再起動）。
